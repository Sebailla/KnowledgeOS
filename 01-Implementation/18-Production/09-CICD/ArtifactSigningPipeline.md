...
**Document:** ArtifactSigningPipeline  
**Version:** 4.0  
...
Define the artifact signing pipeline for KnowledgeOS production, covering build, test, security, signing, release, promotion, deployment and hotfix pipelines.
...
- Packages SHALL be immutable after signing.
- Build inputs SHALL be locked and recorded.
- Container images SHALL use minimal trusted bases and non-root execution where possible.
- Packaging verification SHALL include signatures, checksums, SBOM and manifest consistency.
- Least privilege SHALL apply to services, containers, databases, storage and operators.
- Production services SHALL run without unnecessary capabilities.
- Signing and notarization SHALL use protected keys and auditable procedures.
- Vulnerability findings SHALL be triaged by severity and exploitability.
- Security incidents SHALL preserve evidence while protecting user data.
