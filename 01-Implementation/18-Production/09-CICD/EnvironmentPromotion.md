...
**Project:** KnowledgeOS  
**Section:** Implementation / Production / 09-CICD  
**Document:** EnvironmentPromotion  
**Version:** 4.0  
...
Define the environment promotion for KnowledgeOS production, covering build, test, security, signing, release, promotion, deployment and hotfix pipelines.
...
- Production topology SHALL identify every service, volume, network and trust boundary.
- Failure domains SHALL be explicit and SHALL avoid coupling database and source-file durability.
- Capacity assumptions SHALL include catalog size, publication bytes, workflows, users and provider quotas.
- High availability MAY be introduced only without changing authority semantics.
- Every configuration key SHALL have an owner, schema and default policy.
- Invalid dynamic reload SHALL preserve the prior valid configuration.
- Feature flags SHALL not bypass security, privacy or migration requirements.
- Configuration changes SHALL be auditable without exposing secret values.
- Pipelines SHALL use least-privilege credentials.
- Promotion SHALL reuse immutable artifacts rather than rebuild them.
- A failed blocking gate SHALL prevent promotion.
- Hotfixes SHALL retain signing, SBOM, migration and rollback controls.
