...
**Project:** KnowledgeOS  
**Section:** Implementation / Personal Knowledge Sync / 11-Web  
**Document:** ConflictResolutionWeb  
**Version:** 4.0  
...
Define the conflict resolution web for Personal Knowledge synchronization, covering optional web synchronization integration and limitations.
...
- Entity and version identities SHALL be immutable.
- Sync envelopes SHALL be versioned and self-describing.
- Checkpoints SHALL advance only after durable local application.
- Provider cursors SHALL remain provider-specific implementation details.
- Tombstones SHALL remain retained long enough to guarantee convergence.
- Conflicts SHALL never be resolved by silent last-write-wins unless explicitly approved for that entity type.
- Clients SHALL expose pending, synchronized, conflicted and unavailable states.
- Conflict-resolution UI SHALL preserve both versions until the user or approved merge policy commits a result.
- Background execution SHALL respect platform limits and energy policy.
- UI SHALL use public Sync contracts and SHALL not call providers directly.
- Tests SHALL verify identity preservation across devices.
- Tests SHALL verify offline edits and later convergence.
- Tests SHALL verify tombstone propagation and non-reuse.
- Tests SHALL prove publication payloads and Master Catalog data are excluded.
- Completion SHALL demonstrate that NAS availability is irrelevant to Personal Knowledge synchronization.
