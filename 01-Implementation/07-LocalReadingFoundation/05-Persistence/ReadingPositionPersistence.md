...
**Project:** KnowledgeOS  
**Section:** Implementation / Local Reading Foundation / 05-Persistence  
**Document:** ReadingPositionPersistence  
**Version:** 4.0  
...
Define the reading position persistence for the Local Reading Foundation module, covering local reading session, position and recent-item persistence.
...
- Session, persistent reading position and ephemeral viewport state SHALL remain separate.
- Persisted positions SHALL reference stable anchors or presentation identities, not transient pixel offsets alone.
- Concurrent position updates SHALL preserve the most recent valid personal version according to sync policy.
- Persistence SHALL remain local and independently recoverable.
- Schema changes SHALL have versioned migrations.
- Corrupt derived state SHALL be discarded before authoritative or personal state.
