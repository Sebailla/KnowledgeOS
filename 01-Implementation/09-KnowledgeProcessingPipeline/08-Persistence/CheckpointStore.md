...
**Project:** KnowledgeOS  
**Section:** Implementation / Knowledge Processing Pipeline / 08-Persistence  
...
Define the checkpoint store for the Knowledge Processing Pipeline, covering manifests, checkpoints, journals, artifact registry and retention.
...
- Each retryable stage SHALL use stable operation identity.
- Unknown completion state SHALL be reconciled before retry.
- Checkpoints SHALL be written only at consistent stage boundaries.
- Resume SHALL validate processor and schema compatibility.
- Persistent processing metadata SHALL remain separate from canonical publication content.
- Migrations SHALL preserve identities, manifests, checkpoints and provenance.
- Retention SHALL keep enough history to diagnose and reproduce published versions.
