...
**Project:** KnowledgeOS  
**Section:** Implementation / Knowledge Processing Pipeline / 11-Testing  
**Document:** ProviderContractTests  
**Version:** 4.0  
...
Define the provider contract tests for the Knowledge Processing Pipeline, covering conformance, determinism, idempotency, recovery and security verification.
...
- DTOs SHALL not expose private repository or provider implementation types.
- Long-running commands SHALL return workflow or job identity.
- Providers SHALL remain replaceable behind Integration contracts.
- Provider output SHALL be treated as untrusted until validated.
- Provider identity, version and privacy profile SHALL be recorded.
- Fallback SHALL be explicit and SHALL not change authority semantics.
- Tests SHALL verify determinism, idempotency, failure recovery and compatibility.
- Conformance tests SHALL validate UDM and DPM invariants.
- Completion SHALL prove that derived artifacts can be deleted and regenerated.
