...
**Document:** ApplicationArchitecture  
**Version:** 4.0  
...
Define the application architecture for the KnowledgeOS macOS application, covering macOS application architecture and runtime composition.
...
- Dependency direction SHALL follow Foundation → Domain → Kernel → Platform → Integration → Implementation.
- App bootstrap SHALL validate configuration and compose modules deterministically.
- Concrete Apple frameworks SHALL remain behind implementation adapters where architecture contracts require portability.
