...
**Project:** KnowledgeOS  
**Section:** Implementation / Infrastructure / 12-TestInfrastructure  
**Document:** ContainerizedTestServices  
**Version:** 4.0  
...
Define the containerized test services for KnowledgeOS infrastructure, covering repeatable infrastructure for integration, end-to-end, performance and recovery testing.
...
- Container images SHALL be immutable and versioned.
- Services SHALL support graceful shutdown and truthful readiness.
- CPU, memory and storage limits SHALL be explicit where supported.
- Containers SHALL run with minimum required privileges.
- Pipelines SHALL fail on required validation errors.
- Artifacts SHALL be traceable to source revision and dependency lock state.
- Security and dependency checks SHALL be automated.
- Production deployment SHALL require approved promotion policy.
- Test infrastructure SHALL use synthetic or approved data.
- Test environments SHALL be isolated and reproducible.
- Recovery and migration behavior SHALL be exercised, not only success paths.
- Runbooks SHALL identify detection, diagnosis, mitigation and recovery.
- Incidents SHALL preserve evidence and timeline.
- Maintenance SHALL have rollback and communication plans.
- Service objectives SHALL use measurable indicators.
