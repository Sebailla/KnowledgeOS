# Grant Capability Command

**Project:** KnowledgeOS  
**Section:** Implementation / Plugin Runtime / 07-Contracts  
**Document:** GrantCapabilityCommand  
**Version:** 4.0  
...
Define the grant capability command for the Plugin Runtime, covering commands, queries, events, DTOs and compatibility.
...
- Undeclared capability use SHALL fail before privileged execution.
- Capability grants SHALL be bound to plugin identity and version or declared compatibility range.
- Sensitive capabilities SHALL require explicit user or administrator approval.
- Revocation SHALL take effect before future privileged operations.
- Capability use SHALL generate protected audit records.
- Execution context SHALL include plugin identity, capability grants, correlation and limits.
- Hooks and subscriptions SHALL be cancellable and lifecycle-bound.
- Jobs and workflows SHALL preserve host-owned execution semantics.
- Timeout and cancellation SHALL release plugin resources.
- Crash recovery SHALL not replay non-idempotent effects without reconciliation.
- Public contracts SHALL be immutable and versioned.
- DTOs SHALL not expose private host or provider implementation types.
- Events SHALL follow committed host state.
- Long-running operations SHALL return job or workflow identity.
- Plugin state SHALL remain distinguishable from host Domain state.
- Plugin data stores SHALL be scoped by plugin identity.
- Migrations SHALL preserve user-owned data and capability audit history.
- Retention SHALL distinguish configuration, audit, caches and user-owned content.
