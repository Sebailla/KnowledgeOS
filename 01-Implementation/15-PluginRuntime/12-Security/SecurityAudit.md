...
**Project:** KnowledgeOS  
**Section:** Implementation / Plugin Runtime / 12-Security  
...
Define the security audit for the Plugin Runtime, covering threat model, sandbox, signatures, integrity, isolation and supply chain.
...
- Security controls SHALL assume plugins may be malicious or compromised.
- Secrets SHALL never be exposed without a dedicated approved capability.
- Network destinations SHOULD be constrained by policy.
- Supply-chain metadata and package provenance SHALL be retained.
- Security findings SHALL support immediate disablement and revocation.
- Tests SHALL verify capability denial before privileged execution.
- Tests SHALL verify crash and resource-exhaustion isolation.
- Tests SHALL verify incompatible plugins remain disabled without data loss.
- Completion SHALL demonstrate that uninstall does not corrupt host knowledge.
