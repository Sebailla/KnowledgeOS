...
**Project:** KnowledgeOS  
**Section:** Implementation / Search and Discovery / 06-Contracts  
**Document:** RebuildSearchIndexCommand  
**Version:** 4.0  
...
Define the rebuild search index command for Search and Discovery, covering commands, queries, events, DTOs and compatibility.
...
- Index keys SHALL include source and processor versions.
- Incremental indexing SHALL preserve stable identities and avoid duplicate documents.
- Invalidation SHALL target affected entries or partitions.
- A corrupt index SHALL be discarded or rebuilt rather than treated as authoritative.
- Index manifests SHALL record input versions and generation configuration.
- Contracts SHALL be immutable and versioned.
- Long-running indexing operations SHALL return job or workflow identity.
- Events SHALL follow committed index-manifest state.
- DTOs SHALL not expose provider-specific index types.
- Search UI SHALL invoke public Search contracts.
- Result navigation SHALL use stable identities and anchors.
- Accessibility SHALL cover focus, result count, ranking context and error states.
- UI SHALL distinguish local, remote, personal and derived results.
