# Derived Relationship Review

**Project:** KnowledgeOS  
**Section:** Implementation / Knowledge Graph / 09-SearchAndAI  
**Document:** DerivedRelationshipReview  
**Version:** 4.0  
...
Define the derived relationship review for the Knowledge Graph implementation, covering hybrid search, embeddings, AI reasoning and derived relationships.
...
- Vertex and edge identities SHALL be immutable.
- External identifiers SHALL remain namespaced aliases.
- Temporal validity SHALL remain distinct from processing time.
- Ontology mappings SHALL preserve mapping type and provenance.
- Conflicting assertions MAY coexist when authority or evidence differs.
- Projection rules SHALL be versioned.
- Canonical projection SHALL preserve source-backed authority.
- Personal projection SHALL preserve user ownership.
- Derived projection SHALL record processor or model provenance.
- Invalidation SHALL target affected projections without deleting authoritative inputs.
- Embedding and AI provider identity and version SHALL be recorded.
- Similarity SHALL NOT be interpreted as truth.
- Derived relationship acceptance SHALL be an explicit Domain action.
- Remote processing SHALL use minimum necessary authorized data.
- Personal Knowledge SHALL not be transmitted without explicit policy authorization.
