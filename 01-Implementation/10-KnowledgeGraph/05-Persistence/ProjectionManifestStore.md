...
**Project:** KnowledgeOS  
**Section:** Implementation / Knowledge Graph / 05-Persistence  
**Document:** ProjectionManifestStore  
**Version:** 4.0  
...
Define the projection manifest store for the Knowledge Graph implementation, covering graph storage, indexes, manifests, checkpoints, migrations and recovery.
...
- Graph stores and indexes SHALL be replaceable.
- Projection manifests SHALL identify input and rule versions.
- Checkpoint writes SHALL occur only at consistent boundaries.
- Migrations SHALL preserve identities, authority layers and provenance.
- Backups SHALL be verifiable, while rebuildability remains the preferred recovery path for derived state.
