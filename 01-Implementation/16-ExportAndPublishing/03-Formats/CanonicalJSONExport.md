...
Define the canonical jsonexport for Export and Publishing, covering supported formats, capabilities and explicit loss behavior.
...
- Format-specific behavior SHALL remain behind versioned export adapters.
- Target-format metadata SHALL not replace KnowledgeOS provenance.
- Encoding and language SHALL be explicit.
- Malformed output SHALL fail validation before delivery.
