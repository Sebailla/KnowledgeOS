# Loss Reporting Tests

**Project:** KnowledgeOS  
**Section:** Implementation / Export and Publishing / 14-Testing  
**Document:** LossReportingTests  
**Version:** 4.0  
...
Define the loss reporting tests for Export and Publishing, covering determinism, fidelity, loss, conformance, security and recovery verification.
...
- Each format SHALL declare supported and unsupported source capabilities.
- Fidelity SHALL be evaluated against an explicit profile.
- Unsupported required features SHALL fail or degrade according to profile.
- Loss reports SHALL identify omitted, approximated and externally packaged content.
- Export persistence SHALL remain separate from canonical repositories.
- Artifact retention SHALL be explicit.
- Migrations SHALL preserve output identity, manifests and reports.
- Temporary workspaces SHALL never become the sole surviving output location.
- Tests SHALL verify deterministic output for fixed inputs.
- Tests SHALL verify loss reporting.
- Visual regression SHALL use versioned approved baselines.
- Completion SHALL demonstrate source state remains unchanged after export.
