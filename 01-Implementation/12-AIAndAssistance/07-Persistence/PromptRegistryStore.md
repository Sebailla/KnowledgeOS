# Prompt Registry Store

**Project:** KnowledgeOS  
**Section:** Implementation / AI and Assistance / 07-Persistence  
**Document:** PromptRegistryStore  
**Version:** 4.0  
...
Define the prompt registry store for AI and Assistance, covering artifacts, prompts, manifests, models, usage and retention.
...
- Providers SHALL declare capabilities, limits, privacy profile and health.
- Local and remote providers SHALL implement the same logical task contracts where capabilities overlap.
- Fallback SHALL be explicit and SHALL preserve privacy constraints.
- Provider-specific types SHALL not leak into Platform contracts.
- Prompts and templates SHALL be versioned.
- Artifact dependencies SHALL be recorded.
- Invalid or unsafe outputs SHALL be rejected explicitly.
- Published artifacts SHALL be immutable versions.
- Persistent AI state SHALL remain separate from canonical publication state.
- Usage and cost data SHALL avoid sensitive content.
- Migrations SHALL preserve artifact identity and provenance.
- Retention SHALL distinguish Personal Knowledge from disposable derived artifacts.
