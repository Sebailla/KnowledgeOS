...
# 999. Primary Apple Personal-State Synchronization Profile

For the primary KnowledgeOS product profile:

* synchronization scope is personal user state;
* approved participating clients are macOS, iPhone and iPad;
* the default remote synchronization technology is iCloud/CloudKit through an Integration Provider;
* the NAS Master Library is excluded from personal-state synchronization;
* NAS publication delivery is not modeled as bidirectional synchronization;
* device Libraries are selective local collections, not NAS replicas;
* synchronized personal state shall not be written into the NAS Master Library.

Generic synchronization abstractions in this document remain available for future governed profiles, but they shall not override this primary product profile.

Any future synchronization of personal state with a non-iCloud peer requires an explicit architecture decision.
