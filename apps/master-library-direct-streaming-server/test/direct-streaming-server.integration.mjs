  ...
  const acquisitionReceipts = {
    async accept(request, manifest) {
      ...
      const fingerprint = JSON.stringify({
        ...
        targetLocalLibraryId: request.targetLocalLibraryId,
      });
      const existing = receipts.get(key);
      if (existing) {
        if (existing.fingerprint !== fingerprint) {
          const error = new Error("Acquisition receipt conflict");
          error.name = "AcquisitionReceiptConflictError";
          throw error;
        }
      ...
      const accepted = {
        receipt: {
          acquisitionId: `acquisition:${receipts.size + 1}`,
          idempotencyKey: request.idempotencyKey,
          accepted: true,
        },
        manifest,
      };
      receipts.set(key, { fingerprint, accepted });
      return accepted;
    },
  };
  const catalog =
