...
Multiple Apple devices must operate offline against local state while personal user state converges through the approved iCloud/CloudKit synchronization profile. The NAS Master Library is not a personal-state synchronization peer. Synchronization must survive interruption, duplicate delivery, process termination, partial transfers and divergent personal-state changes.

# 2. Decision

Synchronization of personal state shall be durable, identity-based and reconciliation-driven. The Sync Engine owns synchronization semantics; Integration provides transport and provider capabilities. The default Apple-device profile uses iCloud/CloudKit. Sync operates through explicit sessions, comparison, transfer, verification, application and reconciliation. It shall be idempotent where required and shall not assume exactly-once transport or uninterrupted processes.
