...
This document governs the **Security** concern within the Search Engine subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. The governing protection model is: Identity comes from the host boundary, never from query payloads. Results, caches, telemetry, and saved searches are owner-scoped. Tool and plugin callers receive only explicitly authorized fields. Security checks occur before expensive work and before any durable mutation.
...
Owner and subject identity are supplied by the authenticated host.

Plugins and MCP callers cannot override authorization context through request payloads.
