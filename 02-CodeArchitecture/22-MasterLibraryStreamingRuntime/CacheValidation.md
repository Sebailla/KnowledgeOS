...
Este documento desarrolla **contratos, invariantes, flujo operacional y relación con el resto del subsistema** dentro de runtime de streaming de la Master Library. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para implementa upload y download progresivos, rangos, backpressure y validación de caché.

El objetivo es que cualquier cambio relacionado con **CacheValidation** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con CacheValidation. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para CacheValidation, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En CacheValidation, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para CacheValidation es:
...
La persistencia asociada a CacheValidation debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de CacheValidation incluyen:
...
The publication-version content fingerprint is returned as the ETag.

Clients can use `If-None-Match` to avoid retransferring unchanged authoritative content.
