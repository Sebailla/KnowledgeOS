...
**Project:** KnowledgeOS  
**Section:** Code Architecture / 22-MasterLibraryStreamingRuntime  
**Document:** Backpressure  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/master-library-streaming-runtime`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **presupuestos de recursos, límites, degradación y objetivos medibles** dentro de runtime de streaming de la Master Library. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para implementa upload y download progresivos, rangos, backpressure y validación de caché.

El objetivo es que cualquier cambio relacionado con **Backpressure** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con Backpressure. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Runtime de streaming de la master library se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para Backpressure, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En Backpressure, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/master-library-streaming-runtime`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para Backpressure es:
...
La persistencia asociada a Backpressure debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de Backpressure incluyen:
...
**Implementación principal:** `packages/master-library-streaming-runtime`.

**Documentación rectora:** `02-CodeArchitecture/22-MasterLibraryStreamingRuntime/ModuleSpecification.md`.
...
The runtime checks `ServerResponse.write()`.

When the writable buffer is full, it waits for `drain` before continuing. The initial storage API returns a complete byte buffer; file-streamed reads are added in the next storage optimization.
