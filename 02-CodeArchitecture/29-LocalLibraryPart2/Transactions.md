...
Este documento desarrolla **unidad de trabajo, concurrencia, commit, rollback lógico y eventos derivados** dentro de persistencia y recuperación de biblioteca local. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para integra SQLite, filesystem, scanner, reparación, estadísticas y transacciones.

El objetivo es que cualquier cambio relacionado con **Transactions** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con Transactions. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para Transactions, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En Transactions, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para Transactions es:
...
La persistencia asociada a Transactions debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de Transactions incluyen:
...
The SQLite runtime contract exposes `SqliteTransactionManager.run`.

Acquisition workflows will bind repository writes and manifest updates to one SQLite transaction.
