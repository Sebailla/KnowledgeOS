...
Este documento desarrolla **representación del estado, cálculo incremental y reanudación determinista** dentro de persistencia y recuperación de biblioteca local. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para integra SQLite, filesystem, scanner, reparación, estadísticas y transacciones.

El objetivo es que cualquier cambio relacionado con **PersistentManifest** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con PersistentManifest. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para PersistentManifest, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En PersistentManifest, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para PersistentManifest es:
...
La persistencia asociada a PersistentManifest debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de PersistentManifest incluyen:
...
The current deterministic manifest can be persisted in SQLite and replaced incrementally after local state changes.
