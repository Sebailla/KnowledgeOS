...
**Project:** KnowledgeOS  
**Section:** Code Architecture / 29-LocalLibraryPart2  
**Document:** SQLitePersistence  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/local-library-sqlite y packages/local-library-filesystem`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **modelo físico, constraints, índices, migraciones y semántica transaccional** dentro de persistencia y recuperación de biblioteca local. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para integra SQLite, filesystem, scanner, reparación, estadísticas y transacciones.

El objetivo es que cualquier cambio relacionado con **SQLitePersistence** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con SQLitePersistence. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Persistencia y recuperación de biblioteca local se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para SQLitePersistence, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En SQLitePersistence, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/local-library-sqlite y packages/local-library-filesystem`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para SQLitePersistence es:
...
La persistencia asociada a SQLitePersistence debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de SQLitePersistence incluyen:
...
**Implementación principal:** `packages/local-library-sqlite y packages/local-library-filesystem`.

**Documentación rectora:** `02-CodeArchitecture/29-LocalLibraryPart2/ModuleSpecification.md`.
...
SQLite stores Local Publication records, manifests and settings.

The package depends on an abstract `SqliteExecutor`, keeping native driver selection outside the domain package.
