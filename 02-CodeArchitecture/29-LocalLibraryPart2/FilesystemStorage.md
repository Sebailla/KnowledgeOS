...
Este documento desarrolla **layout físico, direccionamiento, atomicidad de archivos y mantenimiento** dentro de persistencia y recuperación de biblioteca local. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para integra SQLite, filesystem, scanner, reparación, estadísticas y transacciones.

El objetivo es que cualquier cambio relacionado con **FilesystemStorage** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con FilesystemStorage. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para FilesystemStorage, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En FilesystemStorage, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para FilesystemStorage es:
...
La persistencia asociada a FilesystemStorage debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de FilesystemStorage incluyen:
...
Local publication bytes use content-addressed SHA-256 paths with staging and atomic rename.
