...
Este documento desarrolla **contratos públicos, validación, compatibilidad y semántica de errores** dentro de protocolo de cargas reanudables. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para define sesiones, chunks, finalización, durabilidad, limpieza y reanudación idempotente.

El objetivo es que cualquier cambio relacionado con **ChunkProtocol** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con ChunkProtocol. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para ChunkProtocol, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En ChunkProtocol, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para ChunkProtocol es:
...
La persistencia asociada a ChunkProtocol debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de ChunkProtocol incluyen:
...
- are idempotent when checksum and length match;
- carry `x-chunk-sha256`;
- are rejected when an index already contains different content.
