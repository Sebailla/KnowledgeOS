...
```text
Mac creates Personal Knowledge
→ creates sync envelope
→ authenticated HTTP push
→ server stores replica
→ iPhone pulls owner envelope
→ local merge engine applies records
```
