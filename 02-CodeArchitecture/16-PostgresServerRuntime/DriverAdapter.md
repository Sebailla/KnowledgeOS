...
Este documento desarrolla **contratos, invariantes, flujo operacional y relación con el resto del subsistema** dentro de runtime PostgreSQL del servidor. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para compone el driver, migraciones, transacciones, readiness y ciclo de vida de la conexión.

El objetivo es que cualquier cambio relacionado con **DriverAdapter** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con DriverAdapter. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para DriverAdapter, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En DriverAdapter, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para DriverAdapter es:
...
La persistencia asociada a DriverAdapter debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de DriverAdapter incluyen:
...
`PgDatabaseAdapter` converts `pg` Pool and PoolClient behavior into the provider-neutral `SqlDatabase` contract.

The `pg` dependency is isolated in `@knowledgeos/infrastructure-postgres-node`.
