...
Este documento desarrolla **unidad de trabajo, concurrencia, commit, rollback lógico y eventos derivados** dentro de runtime PostgreSQL del servidor. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para compone el driver, migraciones, transacciones, readiness y ciclo de vida de la conexión.

El objetivo es que cualquier cambio relacionado con **TransactionContext** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con TransactionContext. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para TransactionContext, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En TransactionContext, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para TransactionContext es:
...
La persistencia asociada a TransactionContext debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de TransactionContext incluyen:
...
`TransactionContextDatabase` uses Node `AsyncLocalStorage`.

During a Unit of Work:

- repositories automatically use the active transaction;
- nested operations reuse the transaction;
- success commits once;
- failure rolls back once;
