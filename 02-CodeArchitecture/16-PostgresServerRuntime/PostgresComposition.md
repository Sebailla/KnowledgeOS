...
**Project:** KnowledgeOS  
**Section:** Code Architecture / 16-PostgresServerRuntime  
**Document:** PostgresComposition  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/postgres-server-runtime, packages/postgres-driver y adaptadores de infraestructura`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **descomposición interna, dependencias permitidas y flujo de composición** dentro de runtime PostgreSQL del servidor. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para compone el driver, migraciones, transacciones, readiness y ciclo de vida de la conexión.

El objetivo es que cualquier cambio relacionado con **PostgresComposition** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con PostgresComposition. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Runtime postgresql del servidor se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para PostgresComposition, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En PostgresComposition, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/postgres-server-runtime, packages/postgres-driver y adaptadores de infraestructura`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para PostgresComposition es:
...
La persistencia asociada a PostgresComposition debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de PostgresComposition incluyen:
...
**Implementación principal:** `packages/postgres-server-runtime, packages/postgres-driver y adaptadores de infraestructura`.

**Documentación rectora:** `02-CodeArchitecture/16-PostgresServerRuntime/ModuleSpecification.md`.
...
The PostgreSQL server composition replaces in-memory aggregate repositories while preserving:
...
The Master Catalog provider remains in memory until the dedicated Master Library implementation.
