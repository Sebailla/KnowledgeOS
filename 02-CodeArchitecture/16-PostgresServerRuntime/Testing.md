...
Este documento desarrolla **estrategia de pruebas, fixtures, propiedades, fallos y criterios de aceptación** dentro de runtime PostgreSQL del servidor. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para compone el driver, migraciones, transacciones, readiness y ciclo de vida de la conexión.

El objetivo es que cualquier cambio relacionado con **Testing** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con Testing. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En Testing, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para Testing es:
...
La persistencia asociada a Testing debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de Testing incluyen:
...
- fake-pool driver tests;
- migration checksum behavior;
- transaction commit and rollback;
- PostgreSQL health mapping;
- server composition contract.

A real PostgreSQL integration test is prepared for CI using `DATABASE_URL`. Docker was not available in the generation environment.
