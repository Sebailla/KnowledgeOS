...
**Project:** KnowledgeOS  
**Section:** `02-CodeArchitecture/43-SearchEnginePart4`  
**Document:** Deterministic Provider  
**Documentation version:** 5.6.3  
**Status:** Consolidated  
**Subsystem:** Search Engine  
**Implemented by:** `packages/search-domain`, `packages/search-index`, `packages/search-sqlite`, `packages/search-query`, `packages/search-query-sqlite`, `packages/search-ranking`, `packages/search-fuzzy`, `packages/search-embedding`, `packages/search-vector-sqlite`, `packages/search-hybrid`, `packages/search-embedding-jobs`, `packages/search-embedding-jobs-sqlite`, `packages/search-embedding-worker`, `packages/search-graph`, `packages/search-graph-sqlite`, `packages/search-graph-runtime`, `packages/search-runtime`, `packages/search-production`, `packages/search-api`  
**Last reviewed:** 2026-08-02

## 1. Purpose

This document specifies **the deterministic provider concern within the Search Engine subsystem, including contracts, invariants, runtime behavior, persistence, failure handling, and verification**. It is normative for the implementation represented by `43-SearchEnginePart4` and must be read together with the module specification and the repository-wide architectural constraints. The objective is not merely to describe current classes; it is to define the behavior that compatible implementations must preserve.

The module exists to build the offline-first lexical, structured, fuzzy, semantic, hybrid, embedding, and graph-aware retrieval pipeline used by Library, AI, Reader, Workspace, plugins, and MCP. This document narrows that broader purpose to **Deterministic Provider**, establishes its observable guarantees, and records where responsibility begins and ends.

## 2. Scope

Included in scope are domain contracts, validation rules, state transitions, persistence effects, concurrency assumptions, failure classification, recovery behavior, observability, security boundaries, and test obligations directly related to the deterministic provider concern within the Search Engine subsystem, including contracts, invariants, runtime behavior, persistence, failure handling, and verification. The specification applies to local macOS execution, NAS-backed operation, restart recovery, and any server component named in the implementation traceability section.
...
The runtime normalizes and compiles the query, selects retrieval stages, executes them under budgets, fuses ranked lists, applies personalization and reranking, creates snippets, and returns an explainable response. The concern documented here participates in that pipeline as a distinct boundary. Inputs are validated before state changes; authoritative identity comes from the trusted runtime context; durable acknowledgements are emitted only after the required persistence boundary has succeeded.

SQLite FTS5 stores lexical indexes; ordinary SQLite tables store parsed queries, vectors, graph projections, embedding jobs, leases, model versions, and stale-state metadata. The design therefore separates logical state, durable local coordination, authoritative remote state, and disposable caches. This separation is essential for offline-first behavior and deterministic recovery.

## 4. Responsibilities

The implementation is responsible for:

- defining a typed and owner-scoped representation of the deterministic provider concern within the Search Engine subsystem, including contracts, invariants, runtime behavior, persistence, failure handling, and verification;
...
The normative flow for the deterministic provider concern within the Search Engine subsystem, including contracts, invariants, runtime behavior, persistence, failure handling, and verification is:
...
- `packages/search-sqlite`
- `packages/search-query`
- `packages/search-query-sqlite`
- `packages/search-ranking`
- `packages/search-fuzzy`
- `packages/search-embedding`
- `packages/search-vector-sqlite`
- `packages/search-hybrid`
- `packages/search-embedding-jobs`
- `packages/search-embedding-jobs-sqlite`
- `packages/search-embedding-worker`
- `packages/search-graph`
- `packages/search-graph-sqlite`
- `packages/search-graph-runtime`
- `packages/search-runtime`
- `packages/search-production`
- `packages/search-api`
...
- **Knowledge Graph:** consumes or publishes stable contracts without bypassing owner, transaction, or persistence boundaries.
...
- **AI Engine:** consumes or publishes stable contracts without bypassing owner, transaction, or persistence boundaries.
- **Reader:** consumes or publishes stable contracts without bypassing owner, transaction, or persistence boundaries.
- **Plugin SDK:** consumes or publishes stable contracts without bypassing owner, transaction, or persistence boundaries.
- **MCP:** consumes or publishes stable contracts without bypassing owner, transaction, or persistence boundaries.
...
The included local provider is deterministic, offline and dependency-free.

It validates the architecture and tests; it is not intended to replace a production semantic model.
