# Postgre S Q L Schema

**Project:** KnowledgeOS  
**Section:** `02-CodeArchitecture/38-PersonalKnowledgePart5`  
**Document:** Postgre S Q L Schema  
...
This document specifies **the postgre s q l schema concern within the Personal Knowledge subsystem, including contracts, invariants, runtime behavior, persistence, failure handling, and verification**. It is normative for the implementation represented by `38-PersonalKnowledgePart5` and must be read together with the module specification and the repository-wide architectural constraints. The objective is not merely to describe current classes; it is to define the behavior that compatible implementations must preserve.

The module exists to define and synchronize owner-scoped personal knowledge, anchors, revisions, tombstones, conflicts, device replicas, authorization boundaries, and production endpoints while preserving canonical Library isolation. This document narrows that broader purpose to **Postgre S Q L Schema**, establishes its observable guarantees, and records where responsibility begins and ends.

## 2. Scope

Included in scope are domain contracts, validation rules, state transitions, persistence effects, concurrency assumptions, failure classification, recovery behavior, observability, security boundaries, and test obligations directly related to the postgre s q l schema concern within the Personal Knowledge subsystem, including contracts, invariants, runtime behavior, persistence, failure handling, and verification. The specification applies to local macOS execution, NAS-backed operation, restart recovery, and any server component named in the implementation traceability section.
...
- defining a typed and owner-scoped representation of the postgre s q l schema concern within the Personal Knowledge subsystem, including contracts, invariants, runtime behavior, persistence, failure handling, and verification;
...
The normative flow for the postgre s q l schema concern within the Personal Knowledge subsystem, including contracts, invariants, runtime behavior, persistence, failure handling, and verification is:
...
Tables: `pk_devices`, `pk_sync_events`, `pk_sync_cursors`, `pk_refresh_tokens`, `pk_sync_audit`.
