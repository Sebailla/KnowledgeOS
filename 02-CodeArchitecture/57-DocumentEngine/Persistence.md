...
This document governs the **Persistence** concern within the Document Engine subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. The governing persistence model is: Parsed documents and processing metadata are stored in SQLite; authoritative source files and binary assets remain in content-addressed Library storage. Schema names are implementation details, but owner-scoped composite keys, deterministic migrations, uniqueness constraints, and restart-safe writes are architectural obligations.
...
Parsed documents persist locally in SQLite while authoritative source files remain outside the database.
