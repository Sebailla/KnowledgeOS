...
This document governs the **Security** concern within the Document Engine subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. The governing protection model is: Parsers operate with bounded resources and no implicit network access. Untrusted HTML, archives, fonts, scripts, and embedded files are treated as hostile inputs. Security checks occur before expensive work and before any durable mutation.
