...
Este documento desarrolla **modelo físico, constraints, índices, migraciones y semántica transaccional** dentro de runtime PostgreSQL de sincronización. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para persiste manifests, planes, checkpoints y leases para ejecución distribuida.

El objetivo es que cualquier cambio relacionado con **Schema** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con Schema. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En Schema, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para Schema es:
...
La persistencia asociada a Schema debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de Schema incluyen:
...
This document defines schema for durable Master-to-Local synchronization. Canonical publication transfer is isolated from Personal Knowledge synchronization.
