# PersonalKnowledgeBoundary

**Project:** KnowledgeOS  
**Section:** Code Architecture / 28-SynchronizationPostgresRuntime  
**Document:** PersonalKnowledgeBoundary  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/synchronization-postgres-runtime`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **contratos, invariantes, flujo operacional y relación con el resto del subsistema** dentro de runtime PostgreSQL de sincronización. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para persiste manifests, planes, checkpoints y leases para ejecución distribuida.

El objetivo es que cualquier cambio relacionado con **PersonalKnowledgeBoundary** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con PersonalKnowledgeBoundary. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Runtime postgresql de sincronización se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para PersonalKnowledgeBoundary, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En PersonalKnowledgeBoundary, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/synchronization-postgres-runtime`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para PersonalKnowledgeBoundary es:
...
La persistencia asociada a PersonalKnowledgeBoundary debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de PersonalKnowledgeBoundary incluyen:
...
**Implementación principal:** `packages/synchronization-postgres-runtime`.

**Documentación rectora:** `02-CodeArchitecture/28-SynchronizationPostgresRuntime/ModuleSpecification.md`.
...
This document defines personal knowledge boundary for durable Master-to-Local synchronization. Canonical publication transfer is isolated from Personal Knowledge synchronization.
