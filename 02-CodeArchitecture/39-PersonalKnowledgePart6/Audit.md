...
- defining a typed and owner-scoped representation of audit records, actor identity, correlation, retention, and privacy;
...
For **Audit**, an additional invariant applies: the state or score produced by this concern must be reproducible from its declared inputs, configuration, persisted state, and clock values. Any nondeterministic provider result must be recorded with enough provenance to explain why repeated execution may differ.
...
The normative flow for audit records, actor identity, correlation, retention, and privacy is:
...
Device registration, revocation, pushes, pulls and conflict resolutions generate audit records with actor, subject, result and timestamp.
