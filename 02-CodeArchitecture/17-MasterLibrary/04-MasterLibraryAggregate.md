# 04 MasterLibraryAggregate

**Project:** KnowledgeOS  
**Section:** Code Architecture / 17-MasterLibrary  
**Document:** 04 MasterLibraryAggregate  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/master-library y sus contratos de aplicación`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **contratos, invariantes, flujo operacional y relación con el resto del subsistema** dentro de dominio de la Master Library. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para gobierna identidad, versiones, assets, publicaciones, metadatos, procedencia e integridad de la biblioteca autoritativa.

El objetivo es que cualquier cambio relacionado con **04 MasterLibraryAggregate** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con 04 MasterLibraryAggregate. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Dominio de la master library se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para 04 MasterLibraryAggregate, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En 04 MasterLibraryAggregate, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/master-library y sus contratos de aplicación`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para 04 MasterLibraryAggregate es:
...
La persistencia asociada a 04 MasterLibraryAggregate debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de 04 MasterLibraryAggregate incluyen:
...
**Implementación principal:** `packages/master-library y sus contratos de aplicación`.

**Documentación rectora:** `02-CodeArchitecture/17-MasterLibrary/ModuleSpecification.md`.
...
**Project:** KnowledgeOS  
**Section:** Code Architecture / Master Library  
**Version:** 5.0  
**Status:** Release Candidate  

## Purpose

Define the 04 masterlibraryaggregate for the authoritative NAS-hosted Master Library.

## Core Rules

- `KnowledgeObject` is the stable root identity.
- A Publication is a managed manifestation of a Knowledge Object.
- Publication versions are immutable.
- Source Items and assets are content-addressed by fingerprint.
- Deduplication never silently merges conflicting identities.
- PostgreSQL stores catalog and state; authoritative publication bytes remain in the dedicated files volume.
- Personal Knowledge is excluded.
- Local Library membership is excluded.
- Acquisition remains explicit and separate from synchronization.
- Derived indexes and embeddings remain rebuildable.

## Implementation Status

The first executable package provides registration, versioning, deduplication, integrity, snapshots and repository contracts.
