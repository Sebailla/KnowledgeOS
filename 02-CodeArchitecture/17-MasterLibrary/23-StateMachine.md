...
Este documento desarrolla **estados válidos, transiciones, timeouts y reglas de exclusión** dentro de dominio de la Master Library. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para gobierna identidad, versiones, assets, publicaciones, metadatos, procedencia e integridad de la biblioteca autoritativa.

El objetivo es que cualquier cambio relacionado con **23 StateMachine** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con 23 StateMachine. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para 23 StateMachine, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En 23 StateMachine, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para 23 StateMachine es:
...
La persistencia asociada a 23 StateMachine debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de 23 StateMachine incluyen:
...
Define the 23 statemachine for the authoritative NAS-hosted Master Library.
