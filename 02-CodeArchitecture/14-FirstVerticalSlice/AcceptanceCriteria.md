...
**Project:** KnowledgeOS  
**Section:** `02-CodeArchitecture/14-FirstVerticalSlice`  
**Document:** `AcceptanceCriteria.md`  
**Documentation version:** 5.6.1  
**Status:** Consolidated  
**Last updated:** 2026-08-02  
**Implemented by:** `packages/library`, `packages/infrastructure-postgres`, `packages/testing`

## 1. Purpose

Las pruebas se seleccionan por riesgo arquitectónico, no por métricas de cobertura aisladas. Establece la razón de existencia del documento y la decisión de ingeniería que gobierna. Este documento convierte una descripción breve en una especificación verificable y mantiene la terminología de KnowledgeOS.

## 2. Scope

Cubre **Acceptance Criteria** dentro de **Primer vertical slice**. Especifica el primer flujo extremo a extremo, criterios de aceptación, contrato HTTP y verificación de límites. Delimita los componentes, flujos y consumidores cubiertos, evitando convertir el documento en una descripción global del producto. No redefine Product Vision, arquitectura de dominio global ni políticas operativas pertenecientes a otros directorios.
...
Cada contrato público requiere pruebas de conformidad y cada adaptador requiere al menos una prueba con la tecnología real. Los conceptos deben identificarse mediante nombres estables, datos inmutables cuando representan hechos y versiones explícitas cuando forman parte de un contrato persistente. La identidad lógica se mantiene separada de hashes, ubicaciones físicas y representaciones de transporte.
...
- Cada contrato público requiere pruebas de conformidad y cada adaptador requiere al menos una prueba con la tecnología real.
...
> # Acceptance Criteria
> - A valid source registration returns `202`.
> - The Local Library contains exactly one new Knowledge Object.
> - The title and source identity are preserved.
> - Availability is `local-available`.
> - `readableOffline` is true.
> - The Master Library is not required.
> - Personal Knowledge synchronization is not involved.
> - The compiled integration fixture passes.
