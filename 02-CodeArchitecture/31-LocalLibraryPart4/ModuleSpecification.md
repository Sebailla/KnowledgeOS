# Module Specification — 31-LocalLibraryPart4
...
- `CheckpointOrdering.md` — checkpoint representation, ordering, monotonic advancement, persistence, and restart semantics.
- `EndToEndFlow.md` — complete request or worker path, durable boundaries, acknowledgements, and observable outcomes.
- `FailureBoundaries.md` — failure classification, containment boundaries, retryability, observability, and recovery actions.
- `MasterSourceContract.md` — contract with the authoritative Master Library and the conditions required before local acknowledgement.
- `NextStep.md` — implemented boundary, intentionally deferred capabilities, prerequisites, and compatibility constraints.
- `OfflineInvariant.md` — offline behavior, local availability, deferred work, and reconnection guarantees.
- `PersonalKnowledgeBoundary.md` — the personal knowledge boundary concern within the Local Library subsystem, including contracts, invariants, runtime behavior, persistence, failure handling, and verification.
...
- `ResumeSemantics.md` — resumption tokens, persisted state, replay safety, and duplicate suppression.
- `Testing.md` — unit, integration, property, failure-injection, persistence, concurrency, and end-to-end verification.
- `VerifiedCommit.md` — the verified commit concern within the Local Library subsystem, including contracts, invariants, runtime behavior, persistence, failure handling, and verification.
- `WorkerIntegration.md` — worker ownership, leases, cancellation, heartbeat, and hand-off between runtime phases.
