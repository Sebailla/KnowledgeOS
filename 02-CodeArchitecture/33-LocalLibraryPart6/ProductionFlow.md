...
**Project:** KnowledgeOS  
**Section:** `02-CodeArchitecture/33-LocalLibraryPart6`  
**Document:** Production Flow  
**Documentation version:** 5.6.3  
**Status:** Consolidated  
**Subsystem:** Local Library  
**Implemented by:** `packages/local-library`, `packages/local-library-sqlite`, `packages/local-storage`, `packages/local-cache`, `packages/local-maintenance`, `packages/local-repair`, `packages/sync-local-runtime`, `packages/sync-local-sqlite`, `packages/master-library`, `packages/master-library-api`  
**Last reviewed:** 2026-08-02

## 1. Purpose

This document specifies **production composition, dependency wiring, health, startup, shutdown, and failure isolation**. It is normative for the implementation represented by `33-LocalLibraryPart6` and must be read together with the module specification and the repository-wide architectural constraints. The objective is not merely to describe current classes; it is to define the behavior that compatible implementations must preserve.

The module exists to complete the durable offline execution path that moves authoritative library changes between local SQLite state, staged files, workers, and the Master Library without violating verified-commit ordering. This document narrows that broader purpose to **Production Flow**, establishes its observable guarantees, and records where responsibility begins and ends.

## 2. Scope

Included in scope are domain contracts, validation rules, state transitions, persistence effects, concurrency assumptions, failure classification, recovery behavior, observability, security boundaries, and test obligations directly related to production composition, dependency wiring, health, startup, shutdown, and failure isolation. The specification applies to local macOS execution, NAS-backed operation, restart recovery, and any server component named in the implementation traceability section.
...
- defining a typed and owner-scoped representation of production composition, dependency wiring, health, startup, shutdown, and failure isolation;
...
The normative flow for production composition, dependency wiring, health, startup, shutdown, and failure isolation is:
...
- `packages/master-library`
- `packages/master-library-api`
...
```text
register transfer descriptor
→ enqueue sync job
→ scheduler starts plan
→ HEAD Master publication
→ validate ETag and Content-Length
→ load SQLite checkpoint
→ recover .part staging
→ request HTTP ranges
→ append + fsync
→ update SQLite checkpoint
→ verify SHA-256
→ commit Local Library object
→ persist available/readableOffline
```
