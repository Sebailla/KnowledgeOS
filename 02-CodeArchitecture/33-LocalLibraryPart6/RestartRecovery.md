...
This document specifies **restart-safe state discovery, cursor restoration, lease expiry, and continuation**. It is normative for the implementation represented by `33-LocalLibraryPart6` and must be read together with the module specification and the repository-wide architectural constraints. The objective is not merely to describe current classes; it is to define the behavior that compatible implementations must preserve.

The module exists to complete the durable offline execution path that moves authoritative library changes between local SQLite state, staged files, workers, and the Master Library without violating verified-commit ordering. This document narrows that broader purpose to **Restart Recovery**, establishes its observable guarantees, and records where responsibility begins and ends.

## 2. Scope

Included in scope are domain contracts, validation rules, state transitions, persistence effects, concurrency assumptions, failure classification, recovery behavior, observability, security boundaries, and test obligations directly related to restart-safe state discovery, cursor restoration, lease expiry, and continuation. The specification applies to local macOS execution, NAS-backed operation, restart recovery, and any server component named in the implementation traceability section.
...
- defining a typed and owner-scoped representation of restart-safe state discovery, cursor restoration, lease expiry, and continuation;
...
The normative flow for restart-safe state discovery, cursor restoration, lease expiry, and continuation is:
...
SQLite descriptors and checkpoints survive restarts.

Filesystem staging survives restarts. Resume is allowed only when durable staging length equals persisted `receivedBytes`.
