...
This document specifies **checksums, consistency checks, corrupt-state quarantine, and repair workflows**. It is normative for the implementation represented by `33-LocalLibraryPart6` and must be read together with the module specification and the repository-wide architectural constraints. The objective is not merely to describe current classes; it is to define the behavior that compatible implementations must preserve.

The module exists to complete the durable offline execution path that moves authoritative library changes between local SQLite state, staged files, workers, and the Master Library without violating verified-commit ordering. This document narrows that broader purpose to **Integrity**, establishes its observable guarantees, and records where responsibility begins and ends.

## 2. Scope

Included in scope are domain contracts, validation rules, state transitions, persistence effects, concurrency assumptions, failure classification, recovery behavior, observability, security boundaries, and test obligations directly related to checksums, consistency checks, corrupt-state quarantine, and repair workflows. The specification applies to local macOS execution, NAS-backed operation, restart recovery, and any server component named in the implementation traceability section.
...
- defining a typed and owner-scoped representation of checksums, consistency checks, corrupt-state quarantine, and repair workflows;
...
The normative flow for checksums, consistency checks, corrupt-state quarantine, and repair workflows is:
...
The Master descriptor is validated before transfer.

The final Local Publication is committed only after full SHA-256 and byte-length verification.
