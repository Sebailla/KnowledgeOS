...
**Project:** KnowledgeOS  
**Code Architecture module:** `55-AIEngine`  
**Subsystem:** AI Engine  
**Documentation version:** 5.6.4  
**Status:** Consolidated  
**Implemented by:** `packages/ai-context`, `packages/ai-contracts`, `packages/ai-memory`, `packages/ai-memory-sqlite`, `packages/ai-provider`, `packages/ai-runtime`, `packages/ai-tools`

## Executive specification

This document governs the **Privacy** concern within the AI Engine subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. The governing protection model is: Local-first execution is preferred for sensitive content. Remote calls require policy approval and minimized context. Tool execution uses explicit scopes and audited results. Security checks occur before expensive work and before any durable mutation.
...
- Related packages: `packages/ai-context`, `packages/ai-contracts`, `packages/ai-memory`, `packages/ai-memory-sqlite`, `packages/ai-provider`, `packages/ai-runtime`, `packages/ai-tools`
...
The runtime keeps provider selection explicit. Local providers can operate without external data transfer. Remote-provider policies are enforced by the host.
