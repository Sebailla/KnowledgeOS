...
This document governs the **Architecture** concern within the AI Engine subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. It participates in the canonical processing flow: receive user message → load conversation → retrieve authorized context → apply token budget → select provider/model → stream or generate → validate tool calls → persist response and telemetry.
...
Search, Knowledge Graph and Personal Knowledge remain external authoritative sources.
