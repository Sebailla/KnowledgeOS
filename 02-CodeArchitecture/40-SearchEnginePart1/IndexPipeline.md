...
The module exists to build the offline-first lexical, structured, fuzzy, semantic, hybrid, embedding, and graph-aware retrieval pipeline used by Library, AI, Reader, Workspace, plugins, and MCP. This document narrows that broader purpose to **Index Pipeline**, establishes its observable guarantees, and records where responsibility begins and ends.
...
```text
domain event or extraction result
→ SearchIndexCommand
→ ordered incremental consumer
→ transaction
→ document repository
→ FTS5 projection
→ checkpoint
