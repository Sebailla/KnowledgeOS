...
The module exists to build the offline-first lexical, structured, fuzzy, semantic, hybrid, embedding, and graph-aware retrieval pipeline used by Library, AI, Reader, Workspace, plugins, and MCP. This document narrows that broader purpose to **Incremental Indexing**, establishes its observable guarantees, and records where responsibility begins and ends.
...
Each command has a monotonic sequence.

The consumer checkpoint advances only inside the same unit of work as the index mutation.
