# Module Specification — 40-SearchEnginePart1
...
- `IncrementalIndexing.md` — search document projection, incremental index mutations, invalidation, and consistency.
- `IndexPipeline.md` — search document projection, incremental index mutations, invalidation, and consistency.
- `NextStep.md` — implemented boundary, intentionally deferred capabilities, prerequisites, and compatibility constraints.
- `Normalization.md` — Unicode normalization, token boundaries, case folding, language handling, and deterministic indexing.
- `OfflineFirst.md` — offline behavior, local availability, deferred work, and reconnection guarantees.
- `PersonalKnowledgeIndexing.md` — search document projection, incremental index mutations, invalidation, and consistency.
...
- `Ranking.md` — score components, normalization, profiles, deterministic ties, personalization, and explanations.
- `SQLiteFTS5.md` — the s q lite f t s5 concern within the Search Engine subsystem, including contracts, invariants, runtime behavior, persistence, failure handling, and verification.
- `SearchDocument.md` — canonical searchable projection, source identity, fields, metadata, and versioning.
- `Snippets.md` — context selection, offsets, highlighting, format safety, and reproducibility.
