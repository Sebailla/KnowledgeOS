...
Este documento desarrolla **modos de fallo, recuperación idempotente y preservación de invariantes** dentro de worker de sincronización. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para ejecuta planes con leases, cancelación, progreso, reanudación y aislamiento de fallos.

El objetivo es que cualquier cambio relacionado con **FailureHandling** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con FailureHandling. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para FailureHandling, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En FailureHandling, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para FailureHandling es:
...
La persistencia asociada a FailureHandling debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de FailureHandling incluyen:
...
Execution failures mark the plan failed and release the lease.

Completed checkpoints remain completed. Unfinished checkpoints preserve their last committed offset.
