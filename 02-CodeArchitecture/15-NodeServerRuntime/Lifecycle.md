...
Cubre **Lifecycle** dentro de **Runtime de servidor Node.js**. Define ciclo de vida, adaptador HTTP, seguridad, boundary del runtime e integración ejecutable en Node.js. Delimita los componentes, flujos y consumidores cubiertos, evitando convertir el documento en una descripción global del producto. No redefine Product Vision, arquitectura de dominio global ni políticas operativas pertenecientes a otros directorios.
...
> 2. Compose the server application.
> 3. Bind the HTTP listener.
> 4. Expose the actual address.
> 5. Accept requests.
> Shutdown stops accepting new connections and closes the listener. Durable workflow shutdown will be added with the worker runtime.
