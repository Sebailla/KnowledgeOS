...
**Project:** KnowledgeOS  
**Section:** Code Architecture / 18-MasterLibraryPostgres  
**Document:** FingerprintIndex  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/master-library-postgres`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **identidad de contenido, verificación, deduplicación e inconsistencias detectables** dentro de persistencia PostgreSQL de la Master Library. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para mapea agregados y repositorios del dominio a un esquema transaccional e indexado.

El objetivo es que cualquier cambio relacionado con **FingerprintIndex** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con FingerprintIndex. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Persistencia postgresql de la master library se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para FingerprintIndex, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En FingerprintIndex, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/master-library-postgres`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para FingerprintIndex es:
...
La persistencia asociada a FingerprintIndex debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de FingerprintIndex incluyen:
...
**Implementación principal:** `packages/master-library-postgres`.

**Documentación rectora:** `02-CodeArchitecture/18-MasterLibraryPostgres/ModuleSpecification.md`.
...
`content_fingerprint` is unique across Master Publication Versions.

This provides deterministic duplicate detection before publication registration or new-version creation.
