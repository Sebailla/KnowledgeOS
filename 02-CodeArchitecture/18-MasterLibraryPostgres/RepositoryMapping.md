...
Este documento desarrolla **modelo físico, constraints, índices, migraciones y semántica transaccional** dentro de persistencia PostgreSQL de la Master Library. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para mapea agregados y repositorios del dominio a un esquema transaccional e indexado.

El objetivo es que cualquier cambio relacionado con **RepositoryMapping** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con RepositoryMapping. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para RepositoryMapping, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En RepositoryMapping, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para RepositoryMapping es:
...
La persistencia asociada a RepositoryMapping debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de RepositoryMapping incluyen:
...
Repositories implement contracts from `@knowledgeos/master-library`.

SQL rows are mapped to immutable Master Library models. Domain behavior remains outside PostgreSQL.
