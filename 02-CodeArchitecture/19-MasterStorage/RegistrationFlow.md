...
Este documento desarrolla **contratos, invariantes, flujo operacional y relación con el resto del subsistema** dentro de almacenamiento autoritativo de archivos. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para controla staging, commit, layout, integridad, recuperación y objetos huérfanos.

El objetivo es que cualquier cambio relacionado con **RegistrationFlow** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con RegistrationFlow. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para RegistrationFlow, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En RegistrationFlow, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para RegistrationFlow es:
...
La persistencia asociada a RegistrationFlow debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de RegistrationFlow incluyen:
...
stage bytes
→ derive fingerprint
→ register Master Publication
→ commit object
→ save storage catalog reference
```

A production composition will place catalog and Master Library writes inside one coordinated workflow with compensation.
