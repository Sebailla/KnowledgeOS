...
Este documento desarrolla **modos de fallo, recuperación idempotente y preservación de invariantes** dentro de almacenamiento autoritativo de archivos. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para controla staging, commit, layout, integridad, recuperación y objetos huérfanos.

El objetivo es que cualquier cambio relacionado con **FailureRecovery** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con FailureRecovery. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Almacenamiento autoritativo de archivos se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para FailureRecovery, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En FailureRecovery, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/master-storage`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para FailureRecovery es:
...
La persistencia asociada a FailureRecovery debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de FailureRecovery incluyen:
...
**Implementación principal:** `packages/master-storage`.

**Documentación rectora:** `02-CodeArchitecture/19-MasterStorage/ModuleSpecification.md`.
...
- failed staging leaves removable `.part` files;
- failed catalog writes do not mutate existing objects;
- duplicate content reuses the same content-addressed object;
- orphan scanning detects objects without catalog references;
- catalog references can be removed without immediately destroying shared content.
