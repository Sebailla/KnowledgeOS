...
Este documento desarrolla **layout físico, direccionamiento, atomicidad de archivos y mantenimiento** dentro de almacenamiento autoritativo de archivos. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para controla staging, commit, layout, integridad, recuperación y objetos huérfanos.

El objetivo es que cualquier cambio relacionado con **OrphanManagement** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con OrphanManagement. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para OrphanManagement, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En OrphanManagement, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para OrphanManagement es:
...
La persistencia asociada a OrphanManagement debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de OrphanManagement incluyen:
...
The scanner compares files in the objects tree against catalog references.

Detected orphans are reported. Deletion or quarantine is a separate explicit maintenance action.
