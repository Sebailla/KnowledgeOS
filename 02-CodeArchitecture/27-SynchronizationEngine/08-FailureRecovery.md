...
Este documento desarrolla **modos de fallo, recuperación idempotente y preservación de invariantes** dentro de motor de sincronización. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para calcula manifests, diferencias, planes, transferencias, checkpoints y commit local.

El objetivo es que cualquier cambio relacionado con **08 FailureRecovery** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con 08 FailureRecovery. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para 08 FailureRecovery, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En 08 FailureRecovery, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para 08 FailureRecovery es:
...
La persistencia asociada a 08 FailureRecovery debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de 08 FailureRecovery incluyen:
...
Interrupted transfers resume; invalid transfers fail without altering readable local content.
