# 07 PersonalKnowledge

**Project:** KnowledgeOS  
**Section:** Code Architecture / 27-SynchronizationEngine  
**Document:** 07 PersonalKnowledge  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/synchronization-engine`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **contratos, invariantes, flujo operacional y relación con el resto del subsistema** dentro de motor de sincronización. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para calcula manifests, diferencias, planes, transferencias, checkpoints y commit local.

El objetivo es que cualquier cambio relacionado con **07 PersonalKnowledge** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con 07 PersonalKnowledge. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Motor de sincronización se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para 07 PersonalKnowledge, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En 07 PersonalKnowledge, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/synchronization-engine`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para 07 PersonalKnowledge es:
...
La persistencia asociada a 07 PersonalKnowledge debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de 07 PersonalKnowledge incluyen:
...
**Implementación principal:** `packages/synchronization-engine`.

**Documentación rectora:** `02-CodeArchitecture/27-SynchronizationEngine/ModuleSpecification.md`.
...
**Project:** KnowledgeOS  
**Section:** Code Architecture / Synchronization Engine  
**Version:** 5.0  
**Status:** Release Candidate  

Annotations and personal knowledge synchronize separately and never merge into canonical content.

## Invariants

- Master Library publication content is authoritative.
- Local acquisition is explicit.
- Transfer progress never moves backwards.
- Verification precedes offline availability.
- Personal Knowledge remains a separate synchronization channel.
