...
Este documento desarrolla **representación del estado, cálculo incremental y reanudación determinista** dentro de motor de sincronización. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para calcula manifests, diferencias, planes, transferencias, checkpoints y commit local.

El objetivo es que cualquier cambio relacionado con **03 DiffAndPlanning** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con 03 DiffAndPlanning. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para 03 DiffAndPlanning, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En 03 DiffAndPlanning, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para 03 DiffAndPlanning es:
...
La persistencia asociada a 03 DiffAndPlanning debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de 03 DiffAndPlanning incluyen:
...
The planner classifies missing, outdated and current entries and creates deterministic transfer work.
