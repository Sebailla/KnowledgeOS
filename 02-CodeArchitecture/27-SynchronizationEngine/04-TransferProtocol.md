...
Este documento desarrolla **contratos públicos, validación, compatibilidad y semántica de errores** dentro de motor de sincronización. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para calcula manifests, diferencias, planes, transferencias, checkpoints y commit local.

El objetivo es que cualquier cambio relacionado con **04 TransferProtocol** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con 04 TransferProtocol. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para 04 TransferProtocol, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En 04 TransferProtocol, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para 04 TransferProtocol es:
...
La persistencia asociada a 04 TransferProtocol debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de 04 TransferProtocol incluyen:
...
Transfers resume from persisted byte offsets and verify final fingerprint and length before commit.
