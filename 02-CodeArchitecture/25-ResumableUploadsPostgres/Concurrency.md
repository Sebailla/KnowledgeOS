...
El objetivo es que cualquier cambio relacionado con **Concurrency** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con Concurrency. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para Concurrency, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En Concurrency, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para Concurrency es:
...
La persistencia asociada a Concurrency debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de Concurrency incluyen:
...
The database provides the final coordination boundary.

Concurrent completion requests cannot both acquire a valid lease for the same session.
