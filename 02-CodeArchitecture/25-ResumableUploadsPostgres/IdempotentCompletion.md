...
**Project:** KnowledgeOS  
**Section:** Code Architecture / 25-ResumableUploadsPostgres  
**Document:** IdempotentCompletion  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/resumable-uploads-postgres`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **contratos, invariantes, flujo operacional y relación con el resto del subsistema** dentro de persistencia PostgreSQL de cargas reanudables. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para mantiene sesiones, chunks, leases, locks y finalización idempotente.

El objetivo es que cualquier cambio relacionado con **IdempotentCompletion** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con IdempotentCompletion. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Persistencia postgresql de cargas reanudables se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para IdempotentCompletion, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En IdempotentCompletion, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/resumable-uploads-postgres`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para IdempotentCompletion es:
...
La persistencia asociada a IdempotentCompletion debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de IdempotentCompletion incluyen:
...
**Implementación principal:** `packages/resumable-uploads-postgres`.

**Documentación rectora:** `02-CodeArchitecture/25-ResumableUploadsPostgres/ModuleSpecification.md`.
...
`master_upload_completions` records the final publication and version identity.

Completion always checks this table before and after acquiring the lease.
