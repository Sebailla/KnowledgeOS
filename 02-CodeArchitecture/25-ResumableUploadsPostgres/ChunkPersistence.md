...
Este documento desarrolla **modelo físico, constraints, índices, migraciones y semántica transaccional** dentro de persistencia PostgreSQL de cargas reanudables. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para mantiene sesiones, chunks, leases, locks y finalización idempotente.

El objetivo es que cualquier cambio relacionado con **ChunkPersistence** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con ChunkPersistence. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para ChunkPersistence, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En ChunkPersistence, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para ChunkPersistence es:
...
La persistencia asociada a ChunkPersistence debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de ChunkPersistence incluyen:
...
PostgreSQL stores chunk metadata and checksums.

Chunk bytes remain in the upload-session files volume through a `ChunkBlobStore` adapter.
