...
This document governs the **Runtime** concern within the Sync Engine subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. The runtime follows the canonical pipeline: project Library events to changes → persist outbox → plan ordered batch → transport → receive from cursor → validate and detect conflicts → apply through Library → persist cursor and acknowledgement. Startup validates configuration and dependencies before accepting work; shutdown stops new work, drains bounded in-flight operations, persists durable checkpoints, and releases leases.
...
The runtime performs push, pull, apply, conflict detection and cursor advancement.
