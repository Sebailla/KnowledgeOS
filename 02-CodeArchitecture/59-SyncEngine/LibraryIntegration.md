...
This document governs the **Library Integration** concern within the Sync Engine subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. It participates in the canonical processing flow: project Library events to changes → persist outbox → plan ordered batch → transport → receive from cursor → validate and detect conflicts → apply through Library → persist cursor and acknowledgement.
...
Committed Library events are transformed into sync changes without direct storage coupling.
