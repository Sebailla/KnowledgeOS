...
**Project:** KnowledgeOS  
**Section:** Code Architecture / 26-ResumableUploadsProductionRuntime  
**Document:** README  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/resumable-uploads-production-runtime`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **visión rectora, límites, dependencias y mapa de lectura del módulo** dentro de composición productiva de cargas reanudables. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para integra API, persistencia, almacenamiento, limpieza, observabilidad y políticas operativas.
...
Composición productiva de cargas reanudables se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
La implementación reside principalmente en `packages/resumable-uploads-production-runtime`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
**Implementación principal:** `packages/resumable-uploads-production-runtime`.

**Documentación rectora:** `02-CodeArchitecture/26-ResumableUploadsProductionRuntime/ModuleSpecification.md`.
...
# Resumable Uploads Production Runtime

**Version:** 5.0  
**Status:** Release Candidate

Integra PostgreSQL, almacenamiento autoritativo, volumen temporal de chunks, sesiones reanudables, leases distribuidos, finalización idempotente y outbox.

## Volúmenes

- PostgreSQL: estado transaccional y coordinación.
- Master Storage: bytes autoritativos.
- Upload Chunks: datos temporales reanudables.

## Flujo

```text
crear sesión → recibir chunks → adquirir lease → ensamblar → registrar publicación → persistir catálogo → outbox → completar
```
