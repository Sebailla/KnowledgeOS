...
Este documento desarrolla **fronteras de confianza, autorización, validación y protección de datos** dentro de API de la Master Library. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para expone registro, consulta y descarga con límites de seguridad y contratos estables.

El objetivo es que cualquier cambio relacionado con **Security** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con Security. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En Security, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para Security es:
...
La persistencia asociada a Security debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de Security incluyen:
...
The current executable boundary validates structure and returns safe errors.

Authentication, authorization, range requests and rate limiting are intentionally deferred to the secured runtime block.
