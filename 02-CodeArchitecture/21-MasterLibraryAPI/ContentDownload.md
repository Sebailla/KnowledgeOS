...
**Project:** KnowledgeOS  
**Section:** Code Architecture / 21-MasterLibraryAPI  
**Document:** ContentDownload  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/master-library-api`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **contratos, invariantes, flujo operacional y relación con el resto del subsistema** dentro de API de la Master Library. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para expone registro, consulta y descarga con límites de seguridad y contratos estables.

El objetivo es que cualquier cambio relacionado con **ContentDownload** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con ContentDownload. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Api de la master library se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para ContentDownload, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En ContentDownload, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/master-library-api`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para ContentDownload es:
...
La persistencia asociada a ContentDownload debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de ContentDownload incluyen:
...
**Implementación principal:** `packages/master-library-api`.

**Documentación rectora:** `02-CodeArchitecture/21-MasterLibraryAPI/ModuleSpecification.md`.
...
Download resolves publication and version identity through the storage catalog and verifies checksum before returning bytes.

The initial response wraps bytes as Base64. Streaming is the next transport enhancement.
