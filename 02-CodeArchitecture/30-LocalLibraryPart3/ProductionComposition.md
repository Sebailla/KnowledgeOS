...
**Document:** ProductionComposition  
**Version:** 5.6.2  
**Status:** Consolidated  
...
Este documento desarrolla **descomposición interna, dependencias permitidas y flujo de composición** dentro de composición productiva de biblioteca local. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para coordina caché, mantenimiento, runtime Node SQLite, recuperación y compatibilidad de plataforma.

El objetivo es que cualquier cambio relacionado con **ProductionComposition** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con ProductionComposition. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para ProductionComposition, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En ProductionComposition, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para ProductionComposition es:
...
La persistencia asociada a ProductionComposition debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de ProductionComposition incluyen:
...
The local runtime owns one SQLite file and one content-addressed storage directory.

The application may open the Local Library without access to the NAS or Master Library.
