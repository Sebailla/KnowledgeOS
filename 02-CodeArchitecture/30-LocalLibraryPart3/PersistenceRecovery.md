...
Este documento desarrolla **modelo físico, constraints, índices, migraciones y semántica transaccional** dentro de composición productiva de biblioteca local. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para coordina caché, mantenimiento, runtime Node SQLite, recuperación y compatibilidad de plataforma.

El objetivo es que cualquier cambio relacionado con **PersistenceRecovery** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con PersistenceRecovery. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para PersistenceRecovery, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En PersistenceRecovery, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para PersistenceRecovery es:
...
La persistencia asociada a PersistenceRecovery debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de PersistenceRecovery incluyen:
...
SQLite WAL and filesystem staging provide independent recovery boundaries.

Repair reconciles catalog state with stored objects after interruptions or external modifications.
