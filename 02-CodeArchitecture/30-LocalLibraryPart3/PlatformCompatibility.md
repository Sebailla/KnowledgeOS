...
**Document:** PlatformCompatibility  
**Version:** 5.6.2  
**Status:** Consolidated  
...
Este documento desarrolla **contratos, invariantes, flujo operacional y relación con el resto del subsistema** dentro de composición productiva de biblioteca local. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para coordina caché, mantenimiento, runtime Node SQLite, recuperación y compatibilidad de plataforma.

El objetivo es que cualquier cambio relacionado con **PlatformCompatibility** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con PlatformCompatibility. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para PlatformCompatibility, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En PlatformCompatibility, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para PlatformCompatibility es:
...
La persistencia asociada a PlatformCompatibility debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de PlatformCompatibility incluyen:
...
The Node runtime targets desktop and server-side tooling.

Apple platform clients may implement the same SQLite contracts using their native SQLite bindings while preserving schema and repository behavior.
