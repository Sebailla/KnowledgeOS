...
This document governs the **Runtime** concern within the Library Engine subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. The runtime follows the canonical pipeline: authenticate owner → load logical state → validate transaction and expected versions → apply operations → verify integrity → persist objects/relationships → append journal → publish events → optionally snapshot. Startup validates configuration and dependencies before accepting work; shutdown stops new work, drains bounded in-flight operations, persists durable checkpoints, and releases leases.
...
The runtime loads state, applies transactions, checks integrity, persists state and events, and creates validated snapshots.
