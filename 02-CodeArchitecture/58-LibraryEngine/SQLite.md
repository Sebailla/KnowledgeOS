...
This document governs the **Sqlite** concern within the Library Engine subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. The governing persistence model is: The NAS holds authoritative content-addressed files and PostgreSQL holds the Master Library; local SQLite stores offline projections, events, cursors, and parsed/indexed derivatives. Runtime caches are non-authoritative. Schema names are implementation details, but owner-scoped composite keys, deterministic migrations, uniqueness constraints, and restart-safe writes are architectural obligations.
...
SQLite stores logical objects, relationships, events and snapshots. Authoritative file content remains external and content-addressable.
