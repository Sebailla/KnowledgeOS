...
Cubre **Dependency Rules** dentro de **Biblioteca de plataforma**. Define casos de uso, puertos, adaptadores y operaciones iniciales de la biblioteca sobre contratos de dominio. Delimita los componentes, flujos y consumidores cubiertos, evitando convertir el documento en una descripción global del producto. No redefine Product Vision, arquitectura de dominio global ni políticas operativas pertenecientes a otros directorios.
...
> # Dependency Rules
> Library may depend on domain-types, contracts, kernel and domain. It must not depend on infrastructure, HTTP frameworks, databases, provider SDKs or client applications.
