...
- `packages/infrastructure-postgres-node`
...
> # Repository Adapters
> Repository adapters implement interfaces exported by `@knowledgeos/domain`.
> They map aggregate snapshots to JSONB and reconstruct aggregates using rehydration factories.
> SQL driver integration is isolated behind `SqlDatabase` and `SqlExecutor`.
