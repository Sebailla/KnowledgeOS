...
**Project:** KnowledgeOS  
**Code Architecture module:** `53-KnowledgeGraphPart4`  
**Subsystem:** KnowledgeGraph  
**Documentation version:** 5.6.4  
**Status:** Consolidated  
**Implemented by:** `packages/knowledge-graph`, `packages/knowledge-graph-inference`, `packages/knowledge-graph-inference-runtime`, `packages/knowledge-graph-inference-sqlite`, `packages/knowledge-graph-ontology`, `packages/knowledge-graph-ontology-runtime`, `packages/knowledge-graph-ontology-sqlite`, `packages/knowledge-graph-runtime`, `packages/knowledge-graph-sqlite`, `packages/knowledge-graph-traversal`, `packages/knowledge-graph-traversal-runtime`, `packages/knowledge-graph-traversal-sqlite`

## Executive specification

This document governs the **Runtime** concern within the KnowledgeGraph subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. The runtime follows the canonical pipeline: validate graph mutation → persist asserted state → update traversal projection → evaluate enabled inference rules → persist derived facts with provenance → publish graph events. Startup validates configuration and dependencies before accepting work; shutdown stops new work, drains bounded in-flight operations, persists durable checkpoints, and releases leases.
...
- Related packages: `packages/knowledge-graph`, `packages/knowledge-graph-inference`, `packages/knowledge-graph-inference-runtime`, `packages/knowledge-graph-inference-sqlite`, `packages/knowledge-graph-ontology`, `packages/knowledge-graph-ontology-runtime`, `packages/knowledge-graph-ontology-sqlite`, `packages/knowledge-graph-runtime`, `packages/knowledge-graph-sqlite`, `packages/knowledge-graph-traversal`, `packages/knowledge-graph-traversal-runtime`, `packages/knowledge-graph-traversal-sqlite`
...
The runtime validates rules, loads asserted facts, infers results, checks consistency and atomically replaces derivations by rule.
