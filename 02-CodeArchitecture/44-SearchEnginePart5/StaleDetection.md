...
- defining a typed and owner-scoped representation of staleness detection by model, source version, content hash, and processing state;
...
The normative flow for staleness detection by model, source version, content hash, and processing state is:
...
An embedding is current only when its stored fingerprint equals the current Search Document fingerprint.

Changed content produces a new job.
