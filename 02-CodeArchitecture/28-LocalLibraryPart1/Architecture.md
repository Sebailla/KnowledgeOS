...
Este documento desarrolla **descomposición interna, dependencias permitidas y flujo de composición** dentro de núcleo de biblioteca local. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para define manifest, adquisición, acceso offline, integridad y frontera con conocimiento personal.

El objetivo es que cualquier cambio relacionado con **Architecture** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con Architecture. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para Architecture, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En Architecture, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para Architecture es:
...
La persistencia asociada a Architecture debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de Architecture incluyen:
...
The Local Library is authoritative only for local availability, cache state and device-local operational metadata.
