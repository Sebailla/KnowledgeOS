...
Este documento desarrolla **identidad de contenido, verificación, deduplicación e inconsistencias detectables** dentro de núcleo de biblioteca local. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para define manifest, adquisición, acceso offline, integridad y frontera con conocimiento personal.
...
Offline access verifies:

- local object exists;
- SHA-256 matches;
- byte length matches;
- state is available;
- `readableOffline` is true.

Integrity failure marks the record failed and unavailable.
