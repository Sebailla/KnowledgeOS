...
Este documento desarrolla **representación del estado, cálculo incremental y reanudación determinista** dentro de núcleo de biblioteca local. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para define manifest, adquisición, acceso offline, integridad y frontera con conocimiento personal.

El objetivo es que cualquier cambio relacionado con **LocalManifest** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con LocalManifest. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para LocalManifest, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En LocalManifest, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para LocalManifest es:
...
La persistencia asociada a LocalManifest debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de LocalManifest incluyen:
...
The manifest lists canonical publication/version fingerprints currently known by a Local Library.

Entries are sorted deterministically by Publication ID.
