...
Este documento desarrolla **contratos, invariantes, flujo operacional y relación con el resto del subsistema** dentro de streaming directo de archivos. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para transfiere contenido desde el almacenamiento sin materialización completa en memoria.

El objetivo es que cualquier cambio relacionado con **DirectReader** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con DirectReader. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Para DirectReader, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En DirectReader, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para DirectReader es:
...
La persistencia asociada a DirectReader debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de DirectReader incluyen:
...
The reader resolves a storage-catalog record to an absolute content-addressed path.

Before opening a stream it verifies:

- catalog record exists;
- path is a regular file;
- filesystem byte length equals catalog byte length.
