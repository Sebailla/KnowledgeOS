...
**Project:** KnowledgeOS  
**Section:** Code Architecture / 23-MasterDirectFileStreaming  
**Document:** IntegrityBoundary  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/master-direct-file-streaming`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **identidad de contenido, verificación, deduplicación e inconsistencias detectables** dentro de streaming directo de archivos. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para transfiere contenido desde el almacenamiento sin materialización completa en memoria.

El objetivo es que cualquier cambio relacionado con **IntegrityBoundary** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con IntegrityBoundary. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Streaming directo de archivos se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para IntegrityBoundary, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En IntegrityBoundary, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/master-direct-file-streaming`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para IntegrityBoundary es:
...
La persistencia asociada a IntegrityBoundary debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de IntegrityBoundary incluyen:
...
**Implementación principal:** `packages/master-direct-file-streaming`.

**Documentación rectora:** `02-CodeArchitecture/23-MasterDirectFileStreaming/ModuleSpecification.md`.
...
Fast-path downloads verify identity, path and byte length.

Full checksum verification remains:

- mandatory at commit;
- available through explicit storage verification;
- scheduled through integrity maintenance jobs.

This avoids hashing an entire large publication for every range request.
