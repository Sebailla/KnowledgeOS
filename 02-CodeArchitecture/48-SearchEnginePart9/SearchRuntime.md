...
This document governs the **Search Runtime** concern within the Search Engine subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. The runtime follows the canonical pipeline: normalize query → plan stages → execute lexical/semantic/graph retrieval → fuse ranks → rerank → build snippets → stream or return response. Startup validates configuration and dependencies before accepting work; shutdown stops new work, drains bounded in-flight operations, persists durable checkpoints, and releases leases.
...
Ejecuta motores léxico, semántico y de grafo en orden progresivo.
