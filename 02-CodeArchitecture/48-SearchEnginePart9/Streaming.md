...
This document governs the **Streaming** concern within the Search Engine subsystem. It narrows the module-wide guarantees to the contracts, state, algorithms, and operational boundaries implied by this concern. It participates in the canonical processing flow: normalize query → plan stages → execute lexical/semantic/graph retrieval → fuse ranks → rerank → build snippets → stream or return response.
...
Los lotes se publican como NDJSON por etapa.
