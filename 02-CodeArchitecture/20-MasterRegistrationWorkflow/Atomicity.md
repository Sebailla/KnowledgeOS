...
Este documento desarrolla **unidad de trabajo, concurrencia, commit, rollback lógico y eventos derivados** dentro de workflow de registro autoritativo. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para coordina almacenamiento y catálogo con atomicidad lógica, outbox, deduplicación y compensación.

El objetivo es que cualquier cambio relacionado con **Atomicity** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con Atomicity. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En Atomicity, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
El flujo normativo para Atomicity es:
...
La persistencia asociada a Atomicity debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de Atomicity incluyen:
...
PostgreSQL catalog, Master Library state and outbox records share one transaction.

Filesystem rename cannot participate in PostgreSQL ACID. The workflow therefore uses staging, deterministic content paths and reconciliation.
