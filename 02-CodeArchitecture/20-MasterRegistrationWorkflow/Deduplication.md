...
**Project:** KnowledgeOS  
**Section:** Code Architecture / 20-MasterRegistrationWorkflow  
**Document:** Deduplication  
**Version:** 5.6.2  
**Status:** Consolidated  
**Implemented by:** `packages/master-registration-workflow`  
**Last updated:** 2026-08-02

---
## 1. Propósito

Este documento desarrolla **identidad de contenido, verificación, deduplicación e inconsistencias detectables** dentro de workflow de registro autoritativo. No redefine la visión del producto ni sustituye los ADR; traduce esas decisiones a reglas implementables y verificables para coordina almacenamiento y catálogo con atomicidad lógica, outbox, deduplicación y compensación.

El objetivo es que cualquier cambio relacionado con **Deduplication** pueda evaluarse sin depender de conocimiento tácito. La especificación establece qué debe permanecer estable, qué puede evolucionar detrás de contratos y qué evidencia deben aportar las pruebas.

## 2. Alcance

Incluye los contratos, estados, validaciones, interacciones y responsabilidades directamente asociados con Deduplication. Comprende ejecución local y de servidor cuando corresponda, persistencia durable, observabilidad y comportamiento frente a reintentos.
...
Workflow de registro autoritativo se ubica entre los contratos de dominio y los adaptadores operativos. Sus consumidores no deben depender de tablas, rutas físicas ni implementaciones concretas. La dirección de dependencia es desde adaptadores hacia contratos estables.
...
Para Deduplication, el módulo debe:
...
Los conceptos centrales son identidad estable, versión monotónica, estado observable, operación idempotente y procedencia. En Deduplication, toda entidad durable debe poder rastrearse hasta una entrada o evento causal.
...
La implementación reside principalmente en `packages/master-registration-workflow`. La separación recomendada distingue: contratos y modelos; servicio de aplicación; puertos de repositorio o transporte; adaptadores de infraestructura; composición productiva; y pruebas de integración.
...
El flujo normativo para Deduplication es:
...
La persistencia asociada a Deduplication debe utilizar constraints como última línea de defensa. Las transacciones abarcan únicamente recursos que puedan confirmarse atómicamente; cuando intervienen filesystem y base de datos, se usa staging más registro durable, outbox o compensación.
...
Las pruebas de Deduplication incluyen:
...
**Implementación principal:** `packages/master-registration-workflow`.

**Documentación rectora:** `02-CodeArchitecture/20-MasterRegistrationWorkflow/ModuleSpecification.md`.
...
The SHA-256 fingerprint is calculated before Master Library registration.

Existing content returns the canonical publication/version identity and reuses the same content-addressed object.
